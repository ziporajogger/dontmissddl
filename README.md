...
> A zero-server, zero-cost deadline reminder. It watches Feishu groups, email, or WeChat public accounts, extracts deadlines with an LLM, stores them in a Feishu Bitable, and reminds you via Feishu workflows.

一个**零服务器、零成本**的 DDL（截止日期）提醒工具。它监控你选的**信息源**（飞书群聊 / 邮箱 / 公众号，任选一个或多个），用大模型自动提取截止时间，存进**飞书多维表格**，再由**飞书工作流**在到期前提醒你。全程跑在 GitHub Actions 上，无需购买服务器。
...
每天 09:00（GitHub Actions 定时任务，时间可在 cron.yml 修改）
        ...
        └─ 飞书提醒（工作流）
               ├─ 新增 DDL → 发通知（工作流）
               └─ 到期 7 / 3 / 1 天 → 逐条发带「忽略/完成」按钮的卡片（工作流）
```

**关键设计**：收集和提取是代码（GitHub Actions），**默认的存储和提醒都在飞书上**——多维表格当数据库，工作流当定时提醒。代码库里不存任何个人数据，也没有后台服务要维护。

存储和通知都做成**可插拔**：默认「飞书多维表格 + 飞书工作流」，可选换成「Google Calendar 存储」，或用代码把提醒推到其它渠道（飞书 / Telegram / 邮件 / 钉钉 / 企业微信 / ntfy），见下方「可选扩展」。
...
3. 发布应用（可能需要管理员审批）
4. （选了飞书群才需要）把机器人拉进群
5. 多维表格里加「剩余天数」公式列 + 配「工作流」提醒 ← **必做，否则收不到提醒**
6. 启用 GitHub Actions
...
第 4 步  按脚本最后打印的清单完成手动步骤（发布应用、加「剩余天数」公式列、配工作流）
...
飞书侧有三块配置：**自建应用**（写表格 + 可选读群消息）、**多维表格**（存数据）、**工作流**（发提醒）。多维表格由 `setup.py` 自动创建。
...
| 提醒状态 | 文本 | 预留 |
| 添加日期 | 日期 | 入库日期 |
| 剩余天数 | 公式 | `DATEDIF(TODAY(), 截止日期, "D")`，给工作流用 |

> 代码写入时「状态」固定填「待办」，已过期的 DDL 会自动跳过。

### 3. 飞书提醒 —— 工作流（必做）

提醒由多维表格自带的「工作流」实现（「自动化」也能做，但工作流能逐条处理多条记录，统一用工作流）。注意：飞书没有单独的「发送卡片」动作，动作就叫 **「发送飞书消息」**，发出来的就是卡片；卡片底部按钮要手动打开 **「添加底部按钮」**。

**新增 DDL 通知（工作流）**：

1. 触发器 = 定时（每天 09:30）。
2. 「查找记录」——条件 = 添加日期 = 今天 且状态是待办，输出「查找到的所有记录」。
3. 「循环」节点——依次处理每条数据，选「查找到的所有记录」。
4. 循环体内「发送飞书消息」——标题/内容点 ⊕ 引用「当前循环数据」字段（`{{标题}}`、`{{截止日期}}`、`{{描述}}`）。
5. 开「添加底部按钮」，加「完成」「忽略」两个按钮，按钮操作 = 修改记录 → 把「状态」改成「完成」/「忽略」。

**到期提醒 7/3/1 天（拆成 3 个工作流）**：

1. 触发器 = 定时（每天 09:30）。
2. 「查找记录」——条件 = 剩余天数 = 7（= 3、= 1）且状态是待办，输出「查找到的所有记录」。
3. 「循环」节点——依次处理每条数据，选「查找到的所有记录」。
4. 循环体内「发送飞书消息」——引用「当前循环数据」的字段，加底部按钮「完成/忽略」改当前这条记录的状态。

> 这样 7/3/1 共 3 个到期工作流，加上「新增 DDL 通知」一共 4 个工作流。为什么到期提醒要加「循环」节点？一次可能查到多条记录，直接在「发送飞书消息」里发汇总卡片的底部按钮只改第一条；循环能每条记录单独发一条消息、按钮各改各的（注意循环体内不能加「条件判断」，筛选条件都放「查找记录」里）。
>
> 为什么提醒用飞书平台自带的工作流、而不是写代码发通知？因为它们能**零服务器**支持「点按钮改状态」的交互卡片——按钮点击由飞书平台闭环处理，不需要回调服务器。若改成代码发卡片，要保留「一键完成/忽略」就得有一个能收回调的服务，会打破零服务器。

---

## 可选扩展：换存储 / 加通知渠道

以下都是**可选项**，不配就维持上面的默认方案（飞书多维表格 + 飞书工作流）。填了对应 Secret 即自动启用。
...
默认用飞书工作流提醒。想同时（或改）让代码自己把「新增 DDL」「7/3/1 天到期」推到别的渠道，填下面 Secret 即可，可多个同时：
...
> 这些是**单向推送**：能发文字，不能「点按钮改状态」。要保留交互卡片的一键完成/忽略，还是得用飞书工作流（见上文）。两者可并存。
...
**dontmissddl** is a zero-server, zero-cost DDL reminder. It monitors Feishu groups, email (any IMAP mailbox), or WeChat public accounts — pick one or more — extracts deadlines with an LLM, stores them in a Feishu Bitable, and sends reminders via Feishu workflows. Everything runs on GitHub Actions' free tier.

**Pipeline:** GitHub Actions (daily 09:00 Beijing, configurable in `cron.yml`) → poll sources → LLM extraction → write Feishu Bitable → Feishu workflows send reminders (new-DDL notice + 7/3/1-day alerts with "ignore/done" buttons).

**Optional:** store to Google Calendar instead of the Bitable, and/or push notifications via code to Feishu / Telegram / email / DingTalk / WeCom / ntfy. All opt-in via Secrets; the default Feishu-workflow flow stays intact.
...
- **Manual** (no public API exists): create the Feishu app + grant `bitable:app` (required) and, only if monitoring groups, `im:message` + bot; publish the app; add the 剩余天数 formula column; configure the workflow rules; enable Actions.
